...
//Chỉ số ID của các thuộc tính
  ...
  app.put('/api/lastindex/productid/:id', async (req, res) => {
    ...
    try { 
      const updatedIndex = await LastIndex.findOneAndUpdate(
      { ProductID: id }, 
      { $set: { ProductID: id + 1 } }, 
      { new: true }
      );
...
app.put('/api/discountproducts/:discountID/:productID', async (req, res) => {
  ...
  try {
    const updatedDiscountProduct = await DiscountProduct.findOneAndUpdate(
...
app.put('/api/detailbill/:billID/:productID', async (req, res) => {
  ...
  try {
    ...
    const updated = await DetailBill.findOneAndUpdate(
      { BillID: billID, ProductID: productID },
      req.body,{ new: true,runValidators:true }
    );
    res.json(updated);
