...
export default function Bill({ billInfor, listCartItems }) {
  ...
  // Thông tin cần thiết
  const QRText =
    "https://img.vietqr.io/image/TCB-19038211821017-qr_only.png?amount=" +
    billInfor.totalCost +
    "&addInfo=Giao%20Dich%20E%27Mart%20" +
    billInfor.Time +
    "%20ngay%20" +
    billInfor.Date +
    "&accountName=TRAN%20QUANG%20TU";
    ...
    <div className={style.BillContainer}>
      ...
      <div className={style.Content}>
          ...
          return (
            <div className={style.Cost}>
              ...
              {item.Reduce === undefined ? (
                ...
                <p>
                  <p className={style.Ori}>{item.Price}</p>
                  <br />
                  <p>{formatter.format(item.Price * (1 - item.Reduce))}</p>
                </p>
              )}
            </div>
          );
        ...
        <div className={style.Cost}>
          ...
          <p></p>
          <p>{formatter.format(billInfor.totalCart)}</p>
        </div>
        <div className={style.Cost}>
          <p>
            <b>Tiết kiệm được</b>
          ...
          <p></p>
          <p>{formatter.format(billInfor.totalPresent)}</p>
        </div>
        <div className={style.Cost}>
          <p>
            <b>Khuyến mại</b>
          ...
          <p></p>
          <p>{formatter.format(billInfor.totalReduce)}</p>
        </div>
        <div className={style.Cost}>
          <p>
            <b>Phí vận chuyển</b>
          ...
          <p></p>
          <p>{formatter.format(billInfor.ShipCost)}</p>
        </div>
        <div className={style.Cost}>
          <p>
          ...
          <p></p>
          <p>{formatter.format(billInfor.totalCost)}</p>
        </div>
      </div>
