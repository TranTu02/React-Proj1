...
function SearchBar() {
  ...
  const data = DATA.listProducts.filter((item) => {
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.SearchBar}>
      ...
      <input type="text" placeholder="Nhập từ khóa tìm kiếm" value={searchTerm} onChange={handleChange} />
      {searchTerm !== "" && (
        <div className={styles.Related}>
          {data.slice(0, 5).map((item, index) => (
            <div className={styles.Product} key={index} onClick={() => handleDetail(item.ProductID)}>
              <span>
                <p>
          ...
          {data.length > 5 && (
            <span className={styles.BtnMore} onClick={handleSubmit}>
              <p>Xem thêm</p>
            </span>
          )}
        </div>
