...
function NavBar() {
    ...
    const routeCategory = (CateID,TypeID) =>{
        ...
        else if(TypeID !== '') { navigate("/CategoryPage/Category/Brand/" + TypeID + "/Others"); DDlist1.current = false;}
        else if(TypeID === '' && CateID === '')  navigate("/CategoryPage/Category/Brand/Type/Sale");
        else 
        if (DDlist1.current === true) {
        ...
        <div className={style.Navbar}>
            <div className={style.Category} >
                ...
                <ul className={style.DropList1}>
                    <li onClick={()=>routeCategory()}><a>Tất cả sản phẩm</a></li>
                    <li onClick={()=>routeCategory('','')}><a>Sản phẩm khuyến mãi</a></li>
                    {
                        listCat.map( item => {
                            return(
                                <li onClick={()=>routeCategory(item.CategoryID,'')}><a>{item.CategoryName}</a>                                 
                                <div className={style.DropList2} key={item.CategoryID}>
                                    <ul>
                                        {
                                            DATA.listTypesByCategory(item.CategoryID).map(type => {
                                                return(
                                                    <li onClick={()=>routeCategory('',type.ProductTypeID)}><a>{type.ProductType}</a></li>
                                                    );
                                            })
                                        }
                                    </ul>
