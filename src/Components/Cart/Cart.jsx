...
function Cart() {
  ...
  const routePayment = () => {
    for (const key in cartItems) {
      if (listProduct.find((prd) => prd.ProductID === parseInt(key)).Stock < parseInt(cartItems[key])) {
        alert("Sản phẩm trong giỏ vượt quá số lượng sản phẩm hiện sẵn hàng");
        return false;
      }
    }
  ...
  return (
    <div className={style.CartContainer}>
      <div className={style.ContentCartDetail}>
        ...
        <p className={style.TitleNumberCart}>
          Bạn đang có <b>{getTotalCartItems()}</b> sản phẩm trong giỏ hàng
        </p>
        {true && (
          <div className={style.Table}>
            {DATA.ListProductsDetail().map((item, key) => {
              if (cartItems[item.ProductID] > 0) {
                return (
                  <div className={style.Item} key={key}>
                    <img src={item.Image} className={style.Illustration} />
                    <div className={style.NameBox}>
                      <h2>{item.ProductName}</h2>
                      <p>Đơn vị tính : {item.CalculationUnit}</p>
                      {item.present !== undefined && (
                        <span>
                          <img src={DATA.listProducts.find((obj) => obj.ProductID === item.present.ProductID).Image} width={40} height={40} />
                          <p>
                            Mua {item.Require} sản phẩm {item.ProductName} để nhận {item.present.Quantity} sản phẩm {listProduct.find((obj) => obj.ProductID === item.present.ProductID).ProductName}{" "}
                            miễn phí.
                          </p>
                        </span>
                      )}{" "}
                    ...
                    <div className={style.PriceBox}>
                      ...
                      <div className={style.Quantity}>
                        <button type="button" onClick={() => handleDown(item.ProductID)}>
                          -
                        </button>
                        <input type="text" value={cartItems[item.ProductID]}></input>
                        <button type="button" onClick={() => handleUp(item.ProductID)}>
                          +
                        </button>
                      </div>
                    </div>
                    <div className={style.DelBox} onClick={() => handleDel(item.ProductID)}>
                      <p>Xóa</p>
                    </div>
                  </div>
                );
      ...
      <div className={style.OrderSummaryBox}>
        ...
        <p>
          Mua thêm để miễn phí giao hàng từ <b>{formatter.format(300000)} ₫</b>
        </p>
