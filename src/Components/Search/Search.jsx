...
function Search({ k }) {
  ...
  const indexOfLastItem = currentPage * itemsPerPage > filteredList.length ? (currentPage - 1) * itemsPerPage + (filteredList.length % 12) : currentPage * itemsPerPage;
  ...
  return (
    <div className={style.SearchContainer}>
      <div className={style.Heading}>
        <h2>Tìm kiếm</h2>
        <p>
          Có <b>{filteredList.length} sản phẩm</b> cho tìm kiếm{" "}
        </p>
        <div className={style.Line} />
      </div>
      <p className={style.SubText}>
        Kết quả tìm kiếm cho <b>"{k}".</b>
      </p>
